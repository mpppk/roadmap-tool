...
PORT=<port> bun run db:seed  # DB を初期データで初期化（冪等）
```
...
```sh
PORT=<port> bun run db:seed  # 開発用サンプルデータを投入（サーバー起動後に実行・冪等）
...
`seed/` ディレクトリにサンプルデータが格納されています。`bun dev` でサーバーを起動してから `PORT=<port> bun run db:seed` を実行すると以下がインポートされます。CLI の import コマンドを使うため、実行中のサーバーが必要です（`roadmap-tool epics import` と同様の動作）。

| ファイル | 内容 |
|---|---|
| `seed/epics.csv` | サンプル Epic（インフラ・フロントエンド・バックエンド） |
| `seed/features.csv` | サンプル Feature（各 Epic に紐付く作業項目） |
| `seed/members.tsv` | サンプルメンバー（Alice・Bob・Carol） |

さらに、実行時点の現在クォーターから 4 クォーター分が自動で作成されます。実際のプロジェクトに合わせてこれらのファイルを編集してください。

> **Note:** Epics・Features・Members は CLI の import コマンドを経由してインポートされます。Quarters は CLI に import 機能がないため oRPC クライアントで直接作成します。
