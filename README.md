...
- **合計更新時の比例再配分**: `allocations.updateTotal` で機能の月次キャパシティ合計を変更すると、既存のメンバー配分が新旧比率でスケーリングされ、その後各メンバーの残月次キャパシティでキャップされます。割り当てきれない分は「未アサイン」として残ります。
- **個人配分の上限**: `allocations.previewMemberAllocation` で他Feature使用量と割り当て可能量を取得し、1.0を超える場合はUIで解決方法を選択します。`allocations.updateMemberAllocation` は `capacityConflictResolution` に応じて、割り当て可能量まで丸める・1.0超過を許可する・他Featureを比例減少する、のいずれかを実行します。
- **四半期移動**: `allocations.moveQuarter` はキャパシティ合計とメンバー配分を別四半期へ移動し、メンバーの月次キャップを尊重しながらマージします。割り当てきれない分は移動先Featureの未アサインとして残ります。
...
メンバーセルの値を直接変更した場合は、保存前に `allocations.previewMemberAllocation` が呼ばれます。同じメンバー・四半期の合計が1.0を超える入力では、セル付近のポップオーバーに `現在の他Feature` と `割り当て可能` が表示され、次のいずれかを選びます。

| 選択肢 | 挙動 |
|---|---|
| 割り当て可能量で割り当て | 他Featureへの配分済み分を差し引いた残余で保存 |
| 1を超えて割り当て | 入力値をそのまま保存し、合計1.0超過を許可 |
| 他Featureを比例減少 | 対象Featureは入力値で保存し、他Featureの同メンバー・同四半期配分を比例縮小して合計1.0にする |

入力値自体が1.0を超える場合は、確認なしで入力値をそのまま保存します。1.0を超えているメンバー・四半期はFeature画面とMember画面で警告色になります。
