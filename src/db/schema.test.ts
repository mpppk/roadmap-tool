...
describe("DB schema", () => {
  ...
  test("can insert and query a epic", async () => {
  ...
  test("can insert ordered epic links and rejects duplicate urls", async () => {
  ...
  test("can insert month and epic_month allocation", async () => {
  ...
  test("allows case differences and epic/member cross-resource name matches", () => {
    state.sqlite
      .prepare(
        "INSERT INTO epics (name, initiative_id, position, created_at) VALUES (?, ?, ?, ?)",
      )
    ...
    expect(epicNames).toEqual(["Auth", "auth"]);
